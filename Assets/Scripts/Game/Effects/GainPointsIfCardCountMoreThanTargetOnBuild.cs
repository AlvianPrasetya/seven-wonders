...
public class GainPointsIfCardCountMoreThanTargetOnBuild : OnBuildEffect {
	...
	public override void Effect(Player player) {
		GainPointsResolver.Count count = () => {
			...
			foreach (Player comparedPlayer in comparedPlayers) {
				if (player.BuiltCardsByType[cardType].Count <= comparedPlayer.BuiltCardsByType[cardType].Count) {
					return 0;
				}
			}
